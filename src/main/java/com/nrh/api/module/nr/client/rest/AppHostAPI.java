...
public class AppHostAPI extends AppBase {
	...
	public AppHostAPI(APIKeyset keys, Boolean createClient) {
	...
	public AppHostModel show(AppConfig appConfig) throws IOException {
		...
		String sAppId = appConfig.getAppId()
		      .toString();
		String sHostId = appConfig.getHostId()
		      .toString();
	...
	public ArrayList<MetricNameModel> metricNames(MetricConfig metricConfig) throws IOException {
		...
		String sAppId = metricConfig.getAppId()
		      .toString();
		String sHostId = metricConfig.getHostId()
		      .toString();
	...
	public Mono<List<MetricDataModel>> metricDataMono(MetricConfig metricConfig) throws IOException {
		// Create the URL segment
		Integer appId = metricConfig.getAppId();
		String sHostId = metricConfig.getHostId()
		      .toString();
		String dataSegment = URL_DATA_PATH.replace("{application_id}", appId.toString());
		dataSegment = dataSegment.replace("{host_id}", sHostId);
		return metricDataMono(metricConfig, dataSegment);
	}
}